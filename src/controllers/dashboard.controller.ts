...
export const getUserDashboard = async (req: FastifyRequest, reply: FastifyReply) => {
  ...
  try {
    ...
    const newUsersCurrent = await db
      .select({ count: sql<number>`count(*)` })
      .from(users)
      .where(and(gte(users.createdAt, currentWeek.start), lte(users.createdAt, currentWeek.end)));

    const newUsersPrevious = await db
      .select({ count: sql<number>`count(*)` })
      .from(users)
      .where(and(gte(users.createdAt, previousWeek.start), lte(users.createdAt, previousWeek.end)));
    ...
    const deactivatedUsersPreviousCount = deactivatedUsersPrevious[0]?.count || 0;
    ...
    for (let i = 0; i <= 11; i++) {
      ...
      const total = await db
        .select({ count: sql<number>`count(*)` })
        .from(users)
        .where(and(eq(users.active, true), gte(users.createdAt, start), lte(users.createdAt, end)));
      ...
      const deactivated = await db
        ...
        .from(users)
        .where(
          and(eq(users.active, false), gte(users.updatedAt, start), lte(users.updatedAt, end))
        );
    ...
    const recentLogins =
      recentLoginsResponse.map((recentLogin) => ({
